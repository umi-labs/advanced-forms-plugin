...
try {
  ...
  // --ignore-scripts and --legacy-peer-deps keep the sandbox install fast and
  // independent of peerDependencies (we only care about file resolution).
  execSync(
    `npm install --no-save --ignore-scripts --legacy-peer-deps "${tarballPath}"`,
  ...
  // Sanity: installed package.json must not reference ./src
  ...
  // For every exports condition target and for main/types, verify the file
  // physically exists inside the installed package directory.
  const errors = []
  ...
  // Belt-and-suspenders: ask Node's resolver to resolve each public subpath
  // (this catches subtle exports-map mistakes that file-existence wouldn't).
